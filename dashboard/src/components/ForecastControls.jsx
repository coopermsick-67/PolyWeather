...
export default function ForecastControls({ registry, selectedStation, onSelectStation, calibratedIds, showBaseline, onShowBaseline, onCopy }) {
  return <section className="forecast-controls" aria-label="Forecast controls">
    <div className="station-filter"><SlidersHorizontal size={16} aria-hidden="true" /><span>Station</span><LocationPicker stations={registry} value={selectedStation} onChange={onSelectStation} allowAll allLabel="All stations" calibratedIds={calibratedIds} className="station-filter-picker" /></div>
